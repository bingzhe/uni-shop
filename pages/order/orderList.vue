    ...
    >
      ...
      >
        ...
        <scroll-view class="h-full" scroll-y @scrolltolower="loadData">
            ...
            <view class="order-item padding bg-main border-radius">
              <view @click="todetail(item)">
                ...
                <view
                  ...
                  :key="goodsIndex"
                >
                  <image :src="goodsItem.previewImg" mode="aspectFill"></image>
                  <view class="right flex1">
                    <text class="clamp-2">{{ goodsItem.goods_name }} </text>
                    <view class="ft-dark fs-xs padding-top-xs">
                      <text class="margin-right"
                        >× {{ goodsItem.goods_num }}</text
                      >
                      <!-- {{ goodsItem.goods_sku_name || "&nbsp;&nbsp;" }} -->
                    </view>
                    <view class="margin-top-sm">
                      <text class="price ft-main fs-sm">{{
                        goodsItem.price
                      }}</text>
                    </view>
                  </view>
                ...
                <view class="dflex-e">
                  ...
                  <text class="price ft-main">{{ item.price }}</text>
                </view>
              </view>
              ...
              <view class="dflex-b margin-top-sm">
                <view>
                  ...
                  <text
                    class="ft-dark"
                    v-if="item.order_refund_state == '处理中'"
                    >退款处理中</text
                  >
                  <text class="ft-dark" v-else-if="item.order_refund_state">{{
                    item.order_refund_state
                  }}</text>
                  <text class="ft-dark" v-else-if="item.status_text == '待评价'"
                    >已发货</text
                  >
                  <text class="ft-dark" v-else>{{ item.status_text }}</text>
                </view>

                <view class="dflex-e">
                  <view class="dflex" v-if="item.status_text == '待付款'">
                    <!-- <button
                  ...
                  <view class="dflex" v-if="item.status_text == '待发货'">
                    <!-- <button
                      v-if="!item.order_refund_state"
                      class="action-btn border-radius-big bg-main"
                      @click="torefund(item)"
                    >
                      申请退款
                  ...
                  <!-- <button
                    v-if="['待收货', '已完成'].includes(item.status_text)"
                    class="action-btn border-radius-big bg-main"
                    @click="toexpress(item)"
                  >
                    查看物流
                  </button> -->
                  <view class="dflex" v-if="item.status_text == '待收货'">
                    <button
                      class="action-btn border-radius-big bg-main main-btn"
                  ...
                  <!-- <view class="dflex" v-if="item.status_text == '待评价'">
                    <button
                      class="action-btn border-radius-big bg-main main-btn"
                  ...
                  <!-- <view
                    class="dflex"
                    v-if="['已取消', '已完成'].includes(item.status_text)"
                  >
                    <button
                      class="action-btn border-radius-big bg-main main-btn"
                      @click="delOrder(item)"
          ...
          <use-loadmore
            v-if="
              tabItem.orderList.length > 0 && tabItem.loaded && tabItem.hasmore
            "
...
import { getMallOrderList } from "@/api/tuanApi.js";
...
export default {
  ...
  methods: {
    ...
    async loadData(source = "add", loading) {
      ...
      let params = {};
      if (cur_nav.id) {
        params.status = cur_nav.id;
      }
      //   const params = {
      //     status: cur_nav.id,
      //   };

      let { data: result } = await getMallOrderList(params);

      cur_nav.loaded = true;

      cur_nav.orderList = (result.data || []).map((order) => {
        order.status_text = this.formatStatus(order.status);
        if (order.goods_list && order.goods_list.length > 0) {
          order.goods_list.forEach((goods) => {
      ...
      //   cur_nav.orderList = res.data.data;
    ...
    //   uni.navigateTo({
    //     url: `/pages/user/order/order-express?order_id=${item.order_id}`,
    //   });
    // },
    // 已发货
    toreceipt(item) {
    ...
    // toevaluate(item) {
    //   uni.navigateTo({
    //     url: `/pages/user/order/order-evaluate?id=${item.order_id}`,
    //   });
    // },
    formatStatus(status) {
      if (status == 1) {
