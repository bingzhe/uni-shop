        ...
        <view class="czyebox">
          <view class="cash-select">
            ...
            <template v-if="$util.isWeiXin() && !$util.isAndroid()">
              <view
                @click="selectType(2)"
                class="cs-li flex-center-y"
                v-bind:class="{ 'cs-li-select': payType == 2 }"
              >
                <image
                  src="/static/imgs/wxpay.png"
            ...
            <template v-else>
              <view
                @click="selectType(1)"
                class="cs-li icon yticon flex-center-y"
                v-bind:class="{ 'cs-li-select': payType == 1 }"
              >
                <!-- 支付宝支付 -->
                <image
                  src="/static/imgs/alipay.png"
                  style="height: 40rpx; width: 40rpx; margin-right: 10rpx"
                  mode="aspectFit"
                ></image>
                支付宝支付
          ...
          <view class="uni-btn-v uni-common-mt">
            <button class="onstep" @click="requestPayment(usermoney)">
              提交
            </button>
          </view>
        </view>
...
<script>
import { orderBalanceCreateApi, orderPayApi } from "@/api/tuanApi.js";
import { Weixin } from "@/static/js/wx-jssdk.js";
...
export default {
  data() {
    return {
      showWxPay: false, //是否显示微信支付
      loading: false,
      price: "",
      dbled: true,
      providerList: [],
      url: "",
      payType: 1, //支付方式 1支付宝，2 H5微信
      usermoney: 0,
      member_id: 0,
      wxCode: "",
      mobile: "",
  ...
  methods: {
    ...
    // 生成订单
    async orderCreate() {
      let res = await orderBalanceCreateApi({
        // goods_type: 3,
        // relation_id: '',
        price: this.price,
      });
    ...
    async orderPay() {
      ...
      if (this.payType == 2) {
        WeixinJSBridge.invoke(
          ...
          JSON.parse(this.payInfo.payData),
          (res) => {
            uni.navigateBack({});
          }
        );
    ...
    //提交支付
    async requestPayment(usermoney) {
