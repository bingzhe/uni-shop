		...
		<view class="my-wrap ">
			<view class="div1">
				...
				<view class="performance-wrap">
						<view class="cmd-num">{{myCMD}}</view>
						<view class="cmd-unit">CMD</view>
						<view class="cmd-detail">CMD可用元宇宙板块置办数字资产</view>
				</view>
				<!-- 2 -->
				<view class="ticket-wrap" >
					<view class="ticket-wrap-header"> 
						<view class="ticket-title">
								通用券
						</view>
						<view class="ticket-detail" @click="toDetail()" >明细</view>
					</view>
					<!-- <hr> -->
					<!-- 券数量 -->
					<view class="ticket-wrap-content">
						<view class="ticket-wrap-content-flex">
							<view>可用通用券: {{myCount.valid_num}}</view>
							<view>已核销通用券: {{myCount.verify_num}}</view>
						</view>
						<view class="ticket-wrap-content-flex">
							<view>过期通用券: {{myCount.expired_num}}</view>
							<view>转让中通用券: {{myCount.trade_num}}</view>
						</view>
					</view>
				</view>
				<!-- 3 -->
				<view class="explain-wrap" >
					<view class="explain-wrap-header">
						<view class="explain-wrap-title">通用券转换余额说明:</view>
						<view class="explain-wrap-btn" @click="openRate">汇率变动</view>
					</view>
					<view class="explain-wrap-content">
						<view class="explain-wrap-content-item">1通用券 = {{cmdRate}}CMD</view>
						<view class="explain-wrap-content-item">交易收取10%手续费</view>
						<view class="explain-wrap-content-item">交易价格自行填写交易</view>
					</view>
					<!-- <image src="../static/imgs/trade.jpg" style="width: 500rpx;height: 180rpx;margin-left: 40rpx;margin-top: 20rpx;"></image> -->
				</view>
			</view>
		</view>
		...
		<uni-popup ref="popupPwd" type="center">
			<view class="phonePopup">
				<view class="phonePopup_sj">
					<view style="margin-right: 20rpx;">可用券数量：</view>
				</view>
				<view class="phonePopup_sj">
					<view style="margin-right: 20rpx;">交易通用券数量：</view>
					<input v-model="form.sum"  type="number" maxlength="11" placeholder="请输入"
						placeholder-style="font-size: 28rpx;" />
				</view>
				<view class="phonePopup_sj">
					<view style="margin-right: 20rpx;">收款余额：</view>
					<input v-model="form.count"  type="number" maxlength="11" placeholder="请输入"
						placeholder-style="font-size: 28rpx;" />
				</view>
				<view class="phonePopup_sj">
					<view style="margin-right: 20rpx;">交易账号：</view><br>
					<input v-model="form.account"  type="text" placeholder="请输入"
						placeholder-style="font-size: 28rpx;" />
				</view>
				<view class="phonePopup_sj">
					<view style="margin-right: 20rpx;">支付密码：</view><br>
					<input v-model="form.pwd"  type="password" placeholder="请输入"
						placeholder-style="font-size: 28rpx;" />
				</view>
				<view class="butBox">
					<button class="but but_determine">确定</button>
				</view>
			</view>
		</uni-popup>
	</view>
	...
	export default {
		...
		methods: {
			...
			async getSpreadUsers() {
				...
				this.myCMD=res.data.data.rate*res.data.data.valid_num;
				this.cmdRate = res.data.data.rate
			},
			async init() {
			...
			toDetail() {
				...
				// debugger
				this.$util.redirectTo('/page_my/myCoupons', undefined);
			},
			openRate() { // 进入echart
				this.$util.redirectTo('/page_my/echarts')
			}
		}
	}
</script>
...
<style lang="scss" scoped>
	...
	.div1{
		// image:"/static/imgs/112.jpg" ;
		// background-image: url("/static/imgs/112.jpg");
		width: 600rpx;
		margin: auto;
		height: 330rpx;
	}
	.performance-wrap {
		...
		background: url(@/static/imgs/teamAwardBanner.jpg) no-repeat -8rpx -8rpx;
		background-size: 110% 110%;
		margin-top: 80rpx;
		border-radius: 30rpx;
		-webkit-box-reflect: below 0 -webkit-linear-gradient(transparent, transparent 90%, rgba(255, 255, 255, .3));
