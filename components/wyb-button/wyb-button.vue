<template>
	<view :style="{
		...
		<button class="wyb-button" :disabled="disabled" :data-param="dataParam" :hover-stop-propagation="true"
			...
			@touchstart="touch" @tap.stop="onClick" :form-type="formType" @longtap.stop="onLongClick"
	...
	export default {
		...
		methods: {
			...
			RGBChange(color, level, type) {
				...
				// 减淡或加深
				for (var i = 0; i < 3; i++)
					type === 'light' ? rgbc[i] = Math.floor((255 - rgbc[i]) * level + rgbc[i]) : rgbc[i] = Math.floor(rgbc[
						i] * (1 -
						level))
				// rgb转hex
